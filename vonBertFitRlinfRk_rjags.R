...
modelName <- paste(D$Rdir, "CMR_Australia/Model_RlinfRk_L.txt", sep="")
...
if (save == TRUE) saveFname <- paste(D$Rdir, 
                                     "CMR_Australia/RData/vonBertRlinfRk_", 
                                     runDate, ".RData", sep = "")
...
LinfMu <- matrix(data = NA, nrow = 10000, ncol = 4)
for (k in 1:4){
  LinfMu[,k] <- zm[[k]][,colnames(zm[[k]]) == 'LinfMu']
}
Linf2 <- as.vector(LinfMu)
hist(Linf2)
