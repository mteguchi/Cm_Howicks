...
data.include <- tmp$data[, n.caps > 9]
data.include <- data.include[rowSums(data.include)>0, ]

data.exclude <- tmp$data[, n.caps < 10]
...
if (!file.exists("RData/CJS_pt_cov_days_v4.rds")){
  ...
  saveRDS(out.list, file = "RData/CJS_pt_cov_days_v4.rds")
...
```{r}
new.data.df.2 %>% select("1997", "1998", "2004", "2005", "2006", "2007", "2008",
...
if (!file.exists("RData/CJS_pt_cov_days_phi_firstCCL_v4.rds")){
  ...
  saveRDS(out.list, file = "RData/CJS_pt_cov_days_phi_firstCCL_v4.rds")
  
} else {
  tmp <- readRDS(file = "RData/CJS_pt_cov_days_phi_firstCCL_v4.rds")
  jm.3 <- tmp$jags.out
}
...
p dot model at last - I know this isn't going to be good... and it wasn't. Instead, I look at the two survival probabilities model; one for 1997 to 2015 and another for after 2015. 
...
## parameters to monitor - when this is changed, make sure to change
## summary statistics indices at the end of this script.
parameters <- c("mean.p", "mean.phi1", "mean.phi2", "N",
                 "beta0", "beta1", "tau.p")
...
MCMC.params$model.file <- "models/Model_CJS_pt_cov_days_2phis.txt"


if (!file.exists("RData/CJS_pt_cov_days_2phis.rds")){
  jm.4 <- jags(data = jags.data,
             inits = inits,
             parameters.to.save= parameters,
             model.file = MCMC.params$model.file,
             n.chains = MCMC.params$n.chains,
             n.burnin = 5000, #MCMC.params$n.burnin,
             n.thin = 5, #MCMC.params$n.thin,
             n.iter = 10000, #MCMC.params$n.samples,
             DIC = T,
             parallel=T)
  ...
  saveRDS(out.list, file = "RData/CJS_pt_cov_days_2phis.rds")

} else {
  tmp <- readRDS(file = "RData/CJS_pt_cov_days_2phis.rds")
  jm.4 <- tmp$jags.out
}
...
```{r}
# look at trace and posteriors
mcmc_trace(jm.4$samples, c("mean.phi1", "mean.phi2"))
mcmc_dens(jm.4$samples, c("mean.phi1", "mean.phi2"))
```
